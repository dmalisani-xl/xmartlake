...
def test_player_mount_shield(patched_save_doc, monkeypatch: MonkeyPatch):
    ...
    assert modified_turn.final_health == 20
    assert modified_turn.final_shield_enabled == True
...
@patch("app.game.save_doc")
def test_player_dismount_shield(patched_save_doc, monkeypatch: MonkeyPatch):
    patch_settings(monkeypatch)
    player = PLAYER_1.copy()
    ...
    assert modified_turn.final_bullets == 10
    assert modified_turn.final_health == 20
    assert modified_turn.final_shield_enabled == False
    assert modified_turn.origin_fuel == 50
