...
def test_player_tools_repair_normal(patched_save_doc, monkeypatch: MonkeyPatch):
    patch_settings(monkeypatch)
    player = PLAYER_1.copy()
    turn = TURN.copy()
    game = GAME.copy()

    bot_response = "T99999"  # Only matter the first character
    turn.received_response = bot_response
    modified_turn = process_response_of_player(game=game, player=player, turn=turn)
    assert modified_turn.action == "T"
    ...
    assert modified_turn.final_health == 30
...
@patch("app.game.save_doc")
def test_player_tools_repair_with_shield(patched_save_doc, monkeypatch: MonkeyPatch):
    patch_settings(monkeypatch)
    player = PLAYER_1.copy()
